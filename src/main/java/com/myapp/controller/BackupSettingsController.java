...
public class BackupSettingsController extends BaseController {
    ...
    private void handleChooseBackupPath() {
        ...
        chooser.setTitle(I18nUtil.get("backup.chooser.title"));
    ...
    private void handleBackup() {
        ...
        if (backupDir.isEmpty()) {
            showAlert(
                    ...
                    I18nUtil.get("backup.path.empty")
            );
            return;
        }
        ...
        try {
            ...
            showAlert(
                    ...
                    I18nUtil.get("backup.success")
                    //I18nUtil.get("backup.success", backupFile.getAbsolutePath())
            );
        } catch (Exception e) {
            showAlert(
                    I18nUtil.get("alert.error"),
                    I18nUtil.get("backup.error") + ": " + e.getMessage()
            );
        }
    }
    ...
    private void handleChooseRestoreFile() {
        ...
        chooser.setTitle(I18nUtil.get("restore.chooser.title"));
        chooser.getExtensionFilters().add(
                new FileChooser.ExtensionFilter(
                        I18nUtil.get("restore.filter.zip"),
                        "*.zip"
                )
        );
    ...
    private void handleRestore() {
        ...
        if (backupFile.isEmpty()) {
            showAlert(
                    I18nUtil.get("alert.error"),
                    I18nUtil.get("restore.path.empty")
            );
            return;
        }
        ...
        try {
            ...
            if (!file.exists()) {
                showAlert(
                        I18nUtil.get("alert.error"),
                        I18nUtil.get("restore.file.missing")
                );
                return;
            }
            ...
            showAlert(
                    I18nUtil.get("alert.success"),
                    I18nUtil.get("restore.success")
            );
        } catch (Exception e) {
            showAlert(
                    I18nUtil.get("alert.error"),
                    I18nUtil.get("restore.error") + ": " + e.getMessage()
            );
        }
    }
